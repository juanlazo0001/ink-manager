    ...
    function load() {
      apiFetch<VerifyResponse>(`/flash-payment/verify/${token}`)
        .then((data) => {
          ...
          // Same reasoning as DepositResponse's own poll: the
          // checkout.session.completed webhook is usually near-instant, but
          // not guaranteed to have landed by the time Stripe's own redirect
          // completes, so this briefly re-checks rather than showing a
          // stale "Pay now" button for a payment that already succeeded.
          if (justReturnedFromStripe && pollAttempts < 5) {
            pollAttempts += 1
      ...
      <div className="w-full max-w-lg rounded-2xl card-surface border border-border bg-surface p-8">
        ...
        {state === 'ready' && verifyData && !verifyData.selfScheduleToken && !confirmingPayment && (
          <div>
            ...
            <h1 className="text-xl font-semibold text-fg">Complete your flash booking</h1>
