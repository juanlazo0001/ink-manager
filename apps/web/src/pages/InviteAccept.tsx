...
export default function InviteAccept() {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      ...
      // setSession (not a raw localStorage write) -- AuthContext's
      // token/user state only ever initializes from localStorage once, on
      // mount, so writing the token directly to storage without also
      // updating that state left ProtectedRoute's own useAuth().token
      // still null on the very next render, bouncing straight back to
      // /login despite a valid token now existing on disk. Same as a real
      // login, and lands on Profile rather than Dashboard: a fresh account
      // has nothing on the dashboard yet, but has real setup worth
      // finishing (photo, double-check name/phone).
