...
// to clients.ts's own lightweight POST /export. Kept in its own lib file
// (route-thin/lib-heavy, this app's own convention) since it's genuinely a
// different, heavier action -- six joined CSVs plus real signed PDFs,
// streamed into one ZIP -- not a variant of the column-picker export.
...
// Streams a full-data ZIP export (six joined CSVs + signed deposit-form/
// waiver PDFs) for every client matched by `where` directly into `res`.
// Caller is responsible for auth/permission checks and response headers
// (Content-Type: application/zip, Content-Disposition) before calling this.
export async function streamClientFullExport(where: Prisma.ClientWhereInput, res: NodeJS.WritableStream): Promise<void> {
  ...
  const csvEntries: { name: string; stream: ReturnType<typeof stringify> }[] = [
    { name: "clients.csv", stream: clientsCsv },
    { name: "inquiries.csv", stream: inquiriesCsv },
    { name: "sessions.csv", stream: sessionsCsv },
    { name: "gift-cards.csv", stream: giftCardsCsv },
    { name: "deposits.csv", stream: depositsCsv },
    { name: "waivers.csv", stream: waiversCsv },
  ];
