...
Screen furniture (rev C): top cluster (drawer · tasks badge · bell · avatar), search field, then a **controls row** — **Filter dropdown left-aligned, Sort dropdown right-aligned**, both in the chevron + Jura-caps style. Filter options: All / Unread / Needs Action; when a non-default filter is active the control shows the selection in gold (the active-state language the old chips used). Sort menu adopts the options already shipped in the app (per investigation Q14) — no new sort semantics are invented in this series. **Rev G — empty search:** no matches renders `NO CONVERSATIONS FOUND` (Jura caps, muted) plus a **start-new-chat CTA only if a real creation path exists** (no-inert rule): (A) if search surfaces people without threads, per-person `START CHAT WITH {NAME}` rows using the existing find-or-create path team DMs use; else (B) a single CTA opening an existing compose flow seeded with the query; else (C) text-only empty state and the gap escalates. CTA uses the app's standard primary-button treatment — not red; red stays scarce post-reversal. The **FREQUENT strip is removed** — pinned threads are this screen's quick access. Bottom tab bar with the raised red CHAT button is retained. This series adds behavior beneath that furniture:
