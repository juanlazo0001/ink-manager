...
Branch: `explore/client-self-scheduling`, cut from `main` and pushed to `origin` before any code was written. **Merged into `main`** in a later session (see merge commit below) -- the "Reviewer options" section at the end of this entry is left as originally written, for the historical record of what was offered at merge time.
...
## Reviewer options (as originally offered)
